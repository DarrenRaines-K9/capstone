...
export const NewOrder = () => {
    const [customer, setNewCustomer] = useState({fullName: "", email: "", address: "", phoneNumber: ""}) 
    
    const  navigate = useNavigate()

    const handleNext = (event) => {
        event.preventDefault()
        if(customer.fullName && customer.email && customer.address && customer.phoneNumber) {
                const newCustomer = {
                ...
                createCustomer(newCustomer).then((customerObj) => {
                    navigate("/orders/new-order/order-form", {state: { customer : customerObj}}
                )
        })
        } else { 
